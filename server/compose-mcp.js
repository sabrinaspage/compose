...
const TOOLS = [
  ...
  // -------------------------------------------------------------------------
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        profile: { type: 'object', description: 'Triage build profile (needs_prd/needs_architecture/etc.). Set by the plan lifecycle so build can skip fresh triage.' },
        triageTimestamp: { type: 'string', description: 'Triage cache stamp (ISO 8601). Set AFTER design.md is written so build treats triage as a cached no-op.' },
        plannedBy: { type: 'string', description: 'Originating plan session code (e.g. "PLAN-WIDGET"). Marks a plan-authored, build-ready feature.' },
        impact: { type: 'string', enum: ['low', 'medium', 'high'], description: 'Estimated impact, carried from the plan/ideabox estimation step.' },
  ...
  },
  {
    ...
    description: 'Write person canon by op. create adds a person; open_field supports create, fill, and reopen; edge/load_link support create and remove; correct supports divergence pair/clear. Provenance is writer-stamped.',
    inputSchema: {
      type: 'object',
      required: ['op'],
      properties: {
        op: { type: 'string', enum: ['create', 'add_fact', 'correct', 'open_field', 'edge', 'load_link'] },
        slug: { type: 'string' },
        display_name: { type: 'string' },
        section: { type: 'string', description: 'role|life|stated|revealed' },
        text: { type: 'string' },
        channel: { type: 'string', description: 'said|observed|secondhand|inferred' },
        via: { type: 'string', description: 'Source attribution for a secondhand fact; not provenance.' },
        at: { type: 'string', description: 'ISO date (YYYY-MM-DD).' },
        fact_id: { type: 'string' },
        pair_with: { type: 'string', description: 'Pair stated↔revealed divergence on correct.' },
        clear: { type: 'array', description: 'On correct: exactly ["diverges_with"].', items: { type: 'string' } },
        name: { type: 'string', description: 'open_field create branch.' },
        open_field_id: { type: 'string' },
        filled_by: { type: 'string' },
        reopen: { type: 'boolean' },
        reason: { type: 'string' },
        to: { type: 'string', description: 'edge create branch target person slug.' },
        kind: { type: 'string', description: 'edge create branch kind.' },
        edge_id: { type: 'string' },
        remove: { type: 'boolean' },
        fact: { type: 'string', description: 'load_link create branch fact ID.' },
        carries: { type: 'string' },
        load_link_id: { type: 'string' },
        idempotency_key: { type: 'string' },
      },
    },
  },
  {
    ...
    description: 'Write situation-entity canon by op. create adds an entity; owed supports create, give, and reopen; load_link supports create and remove. Provenance is writer-stamped.',
    inputSchema: {
      type: 'object',
      required: ['op'],
      properties: {
        op: { type: 'string', enum: ['create', 'add_fact', 'correct', 'owed', 'load_link'] },
        slug: { type: 'string' },
        display_name: { type: 'string' },
        text: { type: 'string' },
        channel: { type: 'string', description: 'said|observed|secondhand|inferred' },
        ...
        at: { type: 'string', description: 'ISO date (YYYY-MM-DD).' },
        fact_id: { type: 'string' },
        name: { type: 'string', description: 'owed create branch.' },
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        clause_id: { type: 'string', description: 'correct branch clause ID.' },
        text: { type: 'string', description: 'correct branch replacement wording.' },
        joint: { type: 'string', description: 'joint_link create branch joint slug.' },
        joint_link_id: { type: 'string' },
        remove: { type: 'boolean' },
        reason: { type: 'string' },
        clause: { type: 'string', description: 'load_link create branch v<N>#c<N> reference.' },
        carries: { type: 'string' },
        load_link_id: { type: 'string' },
        idempotency_key: { type: 'string' },
      },
