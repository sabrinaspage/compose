...
## COMP-AUDIT: 2026-07 Cockpit Wiring, Completeness & UX Audit — PLANNED

| # | Feature | Description | Status |
|---|---------|-------------|--------|
| 1 | COMP-AUDIT-1 | Fix ContractEditor infinite re-render crash: Pipeline Editor → step → Contracts throws "Maximum update depth exceeded" (uncached useSyncExternalStore snapshot returns a fresh ref each call). Reproducible 100%; recoverable only via PanelErrorBoundary retry. | PLANNED |
| 2 | COMP-AUDIT-2 | Governance Settings modal fake-saves: SettingsModal.jsx writes settings.governance.phases but settings-store.js:134 rejects `governance` as an unknown top-level key, and the modal flashes 'Saved' without awaiting the result. Governance toggles never persist. | PLANNED |
| 3 | COMP-AUDIT-3 | Gate surface trust bugs: nav 'GATES' badge counts resolved+pending (shows 9 when 1 is actually pending); the pending gate plus one historical gate render the phase transition as 'null -> null'. Undermines the approval surface the product is built around. | PLANNED |
| 4 | COMP-AUDIT-4 | 'Pressure Test' button in the item detail panel is a no-op (ContextItemDetail.jsx:106 onPressureTest={() => {}}). Wire it to a real endpoint or remove the control. | PLANNED |
| 5 | COMP-AUDIT-5 | Mobile agent list never clears finished/killed agents: useLiveAgents.js:61 listens for agentSpawned plus 3 never-emitted names while the server emits agentComplete/agentKilled. A done or killed agent shows as running forever on mobile. | PLANNED |
| 6 | COMP-AUDIT-6 | Graph gate popover Kill resolves with no reason (GraphView.jsx:544) — one-click irreversible kill with no rationale, bypassing the kill-reason guard. Contradicts COMP-COCKPIT-6 (COMPLETE). | PLANNED |
| 7 | COMP-AUDIT-7 | Ten build-stream events (qa_scope, step_usage, gate_tier_result/failed/summary, test_review, health_score, capability_violation, idea_suggestion, buildStreamEvent) are broadcast to the agent SSE stream but dropped by the desktop renderer (build-stream-bridge -> MessageCard.jsx:318). Route them to a canonical consumer: this alone revives the dead QA Scope view and surfaces build cost/health/test telemetry. | PLANNED |
