...
| 14 | COMP-MCP-MIGRATION-2-1-1-1 | **`/compose migrate-anon` interactive flow.** Surfaced by `COMP-MCP-MIGRATION-2-1-1`. Walk historical anonymous-numbered ROADMAP rows one at a time and prompt for a feature code (or 'leave anonymous'). For each promoted row, scaffold `feature.json`, attach to the appropriate phase + position, and on next regen the writer replaces the verbatim anonymous row with a typed-feature row. Optional, deferred — anonymous rows already round-trip cleanly via verbatim passthrough (see parent design Decision 3); this ticket exists for the case where a specific historical row earns promotion to a typed feature for queryability or status-flip mutability. | COMPLETE |
| 15 | COMP-MCP-FOLLOWUP-1 | propose_followup throws `module './project-paths.js' does not provide an export named 'resolveRoadmapPath'` at runtime and writes nothing. Root cause: the long-running stdio compose MCP server caches a stale project-paths.js module record (loaded before COMP-PATHS-EXTERNAL added the export); propose_followup lazily `import()`s lib/followup-writer.js only on first call, which links against the stale cached module. On-disk code resolves cleanly (verified). Live fix: reconnect /mcp (reload server). Hardening: wrap the lazy import so a module-skew/missing-export error rethrows with an actionable "stale MCP server — reconnect /mcp" hint instead of a cryptic ESM error, and add a contract test asserting the followup-writer import graph resolves. Surfaced filing COMP-PARITY-3 follow-ups (fell back to add_roadmap_entry). | COMPLETE |
| 16 | COMP-MCP-FOLLOWUP-1-1 | Eager-import lib/followup-writer.js at compose MCP startup (instead of lazily on first propose_followup call) so a genuine module-skew / missing-export break fails fast at server boot rather than surfacing only when a user first calls the tool. | COMPLETE |
...
| # | Feature | Description | Status |
|---|---------|-------------|--------|
| 128 | COMP-TUI-1 | Step pipeline bar: horizontal progress visualization showing all build steps with done/active/pending states. Always visible at top of output. | COMPLETE |
| 129 | COMP-TUI-2 | Gate panel: interactive gate UI with artifact summary, approve/revise/kill as selectable options. Replaces raw readline prompt. | COMPLETE |
| 130 | COMP-TUI-3 | Violation/findings table: formatted table for review findings (severity, file, line, description) instead of raw JSON output. | COMPLETE |
...
| # | Feature | Description | Status |
|---|---------|-------------|--------|
| 136 | COMP-PIPE-EDIT-1 | Step canvas: Drag-and-drop step nodes on a canvas. Each node shows step ID, agent, intent preview. Dependency edges rendered as arrows. Add step via toolbar or context menu. Delete step via node menu. | COMPLETE |
...
| # | Feature | Description | Status |
|---|---------|-------------|--------|
| 178 | COMP-TEST-BOOTSTRAP-1 | Framework detection: check for test config files (jest.config, vitest.config, pytest.ini, go test files, etc.). If none found, classify project by language/framework and select appropriate test runner. | COMPLETE |
...
| 187 | COMP-TRIAGE-6 | Dispatch scorekeeping: persist per-dispatch telemetry (model, effort, tokens, duration, retries, review findings, test outcome) for every subagent/Codex run + ACRR triage-accuracy metric; aggregate the measured model×effort cost curve so routing replaces guesses with data. Owner-directed 2026-07-23 ("we don't know till we have adequate measurement over time"); next item after COMP-JUDGMENT-STORES. | COMPLETE |
| 188 | COMP-TRIAGE-6-1 | Instrument GSD stepDone settlements so GSD dispatches gain acceptance outcomes | PLANNED |
| 189 | COMP-TRIAGE-6-2 | Execute effort on claude routes (SDK effort param) + keep test runs out of the live dispatch ledger | COMPLETE |
| 190 | COMP-TRIAGE-6-3 | Routing integration: feed the measured model×effort curve into resolveAgentConfig defaults | PARKED |
| 191 | COMP-TRIAGE-6-4 | Rotate build-accumulator identity BEFORE the fallible startFresh on fresh-over-failed: today rotation/counter-reset run after startFresh (build.js:2482 vs 2467), so a retry of a failed build whose fresh plan throws emits a DUPLICATE build-actuals failed row under the old build_id with the prior attempt's stale counters, double-counting in ACRR. Surfaced by b6ecbd5 codex belt-and-braces review 2026-07-23 (P2, reachable single-process). Same review noted 2 concurrency-corner findings (P1a finalizeBuildAttempt clobbers a foreign live sidecar on pre-ownership failure; P1b post-verdict ownership non-atomic) — accepted as PID advisory-lock limitations, not in scope here unless concurrent same-feature builds become a real path. | COMPLETE |
| 192 | COMP-TRIAGE-POOL | Same-box cross-project dispatch pooling: user-level rolled-up ledger feeds a machine baseline curve, per-project data shrinks toward it | PARKED |
...
| # | Feature | Description | Status |
|---|---------|-------------|--------|
| 190 | COMP-CAPS-ENFORCE-1 | Normalizer event tap: Extend `result-normalizer.js` to emit structured `tool_use` events (tool name, timestamp) to a callback or event emitter during `runAndNormalize()`. Currently normalizer only returns `{ text, result }` — tool events are consumed but not surfaced. | COMPLETE |
| 191 | COMP-CAPS-ENFORCE-2 | Violation checker: After each step completes, compare observed tool_use events against the step's agent template (`allowedTools`/`disallowedTools`). Classify: VIOLATION (disallowed tool used), WARNING (tool not in allowedTools but not explicitly disallowed). | COMPLETE |
| 192 | COMP-CAPS-ENFORCE-3 | Build audit integration: Violations written to build-stream as `capability_violation` events with step, agent, tool, template, severity. Surface in review findings and context panel audit trail. | COMPLETE |
| 193 | COMP-CAPS-ENFORCE-4 | Enforcement mode: Policy setting `capabilities.enforcement: "log" \| "block"`. Log mode (default): violations recorded but don't block. Block mode: violation fails the step with `capability_violation` error. | COMPLETE |
...
| # | Feature | Description | Status |
|---|---------|-------------|--------|
| 205 | COMP-OBS-STREAM-1 | Connector enrichment: both connectors yield enriched `tool_use_summary` with `output` field (≤2KB). `result-normalizer.js` forwards to streamWriter (currently missing). Bridge adds `tool_use_summary` case to `_mapEvent`. | COMPLETE |
| 206 | COMP-OBS-STREAM-2 | UI rendering: AgentStream pre-groups consecutive `tool_use` → `tool_use_summary` pairs. MessageCard renders `ToolResultBlock` (new component) attached below tool_use blocks. Collapsible: summary one-liner → first 20 lines → full content. Error detection with destructive styling. | COMPLETE |
| 207 | COMP-OBS-STREAM-3 | Verbose gating: reuses COMP-OBS-SURFACE's verbose toggle (no separate toggle). Verbose off: summaries filtered, no results visible. Verbose on: summaries consumed by pre-grouping, results render attached to tool_use. `tool_progress` renders standalone dimmed. | COMPLETE |
...
| # | Feature | Description | Status |
|---|---------|-------------|--------|
| 1 | COMP-MIMO-INTEROP | **MiMoCode as a second host + neutral benchmark subject.** Promoted from forge-top 2026-06-14 (`MIMO-INTEROP`). MiMoCode (OpenCode fork, 2026-06-10) keeps MCP + the Claude-Code SKILL.md/command/agent format + "Import from Claude Code" → interop substrate. Sanity-checked vs source: compose (`@modelcontextprotocol/sdk` StdioServerTransport, `server/compose-mcp.js:820`) + stratum-mcp (FastMCP/`mcp>=1.0`) register/list/call **unchanged** under any stdio-MCP host; full flows need host-surface adapters (host Agent/Skill dispatch, Compose's separate :4001 HTTP server, skill tool-name mapping). Leg (b): MiMo's self-reported "beats Claude Code at 200+ step tasks" → first external COMP-BENCH subject. Low urgency. See `docs/features/COMP-MIMO-INTEROP/plan.md`. | PLANNED |
| 1 | COMP-PATHS-EXTERNAL | **Relocatable artifact paths — ROADMAP + features (+ journal/context/ideabox) can live outside the workspace root, e.g. a shared `smart-memory-docs` repo.** `.compose/` (config + state) stays at the root; only artifacts redirect. Centralizes path resolution: one default table + one pure resolver (`resolvePathValue` using `path.resolve`, not `join`, so absolute/`../`-escaping overrides work) and sweeps ~25 hardcoded/`join`-based sites. Adds `paths.roadmap`. Default-identity contract: byte-identical when keys unset. v1 writes external artifacts and logs a 'commit it there' line but does NOT auto-commit a separate repo (cross-repo commit = COMP-PATHS-EXTERNAL-1). See `docs/features/COMP-PATHS-EXTERNAL/design.md`. | COMPLETE |
| 2 | COMP-PATHS-EXTERNAL-1 | **Cross-repo auto-commit of external artifacts.** Fast-follow to COMP-PATHS-EXTERNAL. When ROADMAP/features resolve into a separate git repo (e.g. `smart-memory-docs`), the build ship step currently writes them but cannot commit them (git sees only the workspace repo). This ticket teaches ship to commit the external repo via `git -C <external>`, with safety for unrelated dirty files in that repo (stage only Compose-owned paths, never a blanket `git add -A`). Depends on COMP-PATHS-EXTERNAL. | PLANNED |
| 3 | COMP-MCP-ENFORCE-XREPO | Cross-repo completion evidence for the lifecycle guard. record_completion / set_feature_status cannot complete an externally-owned (cross-repo) feature: verifyCompletionEvidence (server/lifecycle-guard.js) git-verifies commit_sha against the COMPOSE repo only, so a feature whose implementing commit lives in another repo (e.g. a stratum-owned STRAT-* feature tracked in compose's roadmap) fails with COMPLETION_EVIDENCE_REQUIRED even though it shipped + passed tests. Add an external-evidence path: accept external_repo/external_commit (verify in the named repo), or skip git-verify for externalPrefixes-matched features while still requiring attested tests + a provenance note. Surfaced by the STRAT-AGENT-INTERP canon backfill, which had to set status COMPLETE via direct feature.json edit because the guard path was unavailable. Owner: compose. | PLANNED |
| 3 | COMP-UITEST-ISOLATION-1 | Harden the Vitest UI suite against parallel-execution flakes. A full `npx vitest run` intermittently fails ~79 tests in `test/ui/mobile-*`, `mobile-remote-auth`, and `agent-stream-remote-mode` (hook timeouts under parallel load); the same files pass in isolation and the suite is green on re-run. Investigate shared global/timer/port state, tighten per-file isolation or hook timeouts, so a single full run is deterministic. Surfaced during COMP-PARITY-3 verification (a flaky run produced a false 79-failure report). | COMPLETE |
| 4 | COMP-ITER-BUDGET | Budget ceilings on iteration loops: configurable max iterations / wall-clock / action-count caps on start_iteration_loop with auto-abort + structured failure report (promoted from idea_budget_ceilings; validated by Claude Code Workflow token-budget runtime, 2026-07) | PLANNED |
...
| 11 | COMP-PARITY-3-1 | Actionable remediation from the environment-health panel: guarded one-click fixes for the read-only drift signals — install a missing dep, run `compose update` when behind, and re-run `compose hooks install` to repair stale/missing hooks (overwriting a foreign hook stays confirm-gated). Reuses existing CLI/endpoint machinery and the same remote-auth posture as COMP-PARITY-3's read endpoint; no new lifecycle. Surfaced by COMP-PARITY-3 (shipped read-only by design). | COMPLETE |
| 11 | COMP-PARITY-8 | UI launchers for build-all and gsd. The cockpit can start a single build but has no trigger for `compose build --all` (build every PLANNED feature) or `compose gsd <CODE>` (per-task fresh-context dispatch); both are CLI/headless-only. Add cockpit controls reusing the existing build-start dispatch so a UI-first user can run a roadmap-wide or GSD build. | COMPLETE |
| 12 | COMP-PARITY-3-2 | Mobile-surface environment-health indicator. COMP-PARITY-3 added the health dot to the desktop cockpit header only; the mobile PWA (`src/mobile/`) is a separate surface and got no signal. Add a lightweight health indicator to the mobile shell consuming the same `GET /api/environment-health` endpoint (read-only). Surfaced by COMP-PARITY-3 (mobile was an explicit non-goal). | COMPLETE |
| 12 | COMP-PARITY-9 | UI feature scaffolding. No cockpit affordance scaffolds a feature folder (equivalent of `compose feature <CODE>`); the only UI path is ideabox promote (COMP-IDEABOX-6), which requires an existing idea as the entry point. Add a New Feature dialog that scaffolds docs/features/<CODE> plus feature.json plus the ROADMAP row directly from the cockpit. | COMPLETE |
| 13 | COMP-PARITY-10 | qa-scope cockpit panel. `compose qa-scope <CODE>` (changed files to affected routes) is terminal-only; COMP-QA shipped the diff-to-route mapper as a build-phase step but there is no standalone cockpit view. Surface affected-route analysis in the cockpit backed by the existing qa-scoping logic so UI-first review can see what a change touches. | COMPLETE |
...
| 291 | COMP-CLI-GLOBAL-FLAGS | Pre-subcommand flag parser to enable compose --workspace=X build syntax | COMPLETE |
...
| 293 | COMP-MOBILE-REMOTE | Remote-reachable transport for COMP-MOBILE: 0.0.0.0 binding (opt-in), short-lived JWT access tokens with refresh, QR + URL + cockpit pairing modal, BYO tunnel (Tailscale/Cloudflare/ngrok). Server-side auth and pairing UI; tunnel layer left to user. | COMPLETE |
...
| 299 | COMP-BUILD-QUICK-1 | Exempt `--quick`-built features from the MISSING_COMPLETION_REPORT validator warning. The quick lifecycle omits the report phase by design, so every quick-built COMPLETE feature would otherwise trip the report-completion check. Ship stamps `built_via:'build-quick'` onto feature.json (via recordCompletion, before the status flip) and feature-validator skips the finding for marked features. Only the report check is exempted — a journal entry is still owed. Follow-up to COMP-BUILD-QUICK. | COMPLETE |
| 300 | STRAT-HERMES-CONNECTOR | Add a Nous Hermes Agent backend to Stratum so compose pipelines can dispatch steps to Hermes (e.g. `agent: hermes`) the way they already do `claude`/`codex`. New `HermesConnector(AgentConnector)` in stratum-mcp/src/stratum_mcp/connectors/ modeled on the existing opencode.py template (spawn-and-translate): drives Hermes headlessly via `hermes -z "<prompt>"` (one-shot, final-text-out), scrubs Nous/Hermes key vars into SENSITIVE_ENV_VARS, implements interrupt() (SIGTERM→SIGKILL). Tier A = text-return (usable as a Codex-style reviewer immediately, and as an executor without live tool narration); Tier B = streaming per-tool envelope events for full cockpit narration, GATED on an unverified Hermes structured-event-stream capability (hermes chat -q gives transcript but JSON event stream unconfirmed). DEPENDS ON T2-F5-OPENCODE-DISPATCH: the connector factory currently hard-restricts to {claude, codex} and raises ValueError for any other type, so the multi-connector dispatch path must be finished first (opencode connector exists but is likewise gated). Compose-side follow-on (small): allow agent:hermes in stratum_agent_run type-validation + pipelines; tier→model mapping (model-tiers.js is Claude-centric) and caps profiles need a hermes branch. Connectors are owned by Stratum (stratum-mcp); tracked here in compose's unified roadmap alongside STRAT-PAR/REV/GUARD. | PLANNED |
| 301 | COMP-CODEX-IMPL | Let Codex be the implementation agent (instead of all-Opus), keeping cross-model review. Codex is already a dispatchable backend, so a `build-codex.stratum.yaml` variant (or `--agent codex` flag, mirroring `--quick`) flips implementation steps to `agent: codex`. Must pair Codex-implements with **Claude**-reviews (not codex reviewing its own work). Frictions: role-aware fix-routing (build.js:3124 codex→claude swap would hijack codex fixes), codex Seatbelt vs the execute step's worktree isolation (spike — start non-parallel), Claude-centric caps/tool enforcement, tier→model mapping. All compose-side; no Stratum work. | COMPLETE |
| 302 | STRAT-AGENT-INTERP | Make Stratum's per-step `agent:` field interpolatable so a flow can select its executor at runtime. Today `Step.agent` is a literal string emitted verbatim by the executor (spec.py:102); `inputs` are rendered (`$.input.*` / `$.steps.*.output.*`) but the agent field is not. Resolve the agent field through the same resolver as inputs before `make_agent_connector`, so `agent: "$.input.executor"` or `agent: "$.steps.route.output.agent"` (a router/triage step emitting the agent for a later step) works. Stays in the data plane: the resolved value comes from recorded flow state, so audit/resume/result-cache replay identically — no control-plane hook, no non-deterministic external actor in the executor loop. Backward-compatible: literal agent strings (no `$.`) behave exactly as today; a resolved value must validate to a known agent type or the step errors before dispatch. Enables spec-controlled dynamic executor selection (e.g. claude vs codex per task) — the data-plane alternative to a mid-flow orchestrator override, and a natural enabler for COMP-CODEX-IMPL. Owner: Stratum (stratum-mcp executor/IR); compose consumes transparently since build.js already honors response.agent. | COMPLETE |
| 303 | COMP-BUILD-RESUME | Explicit `compose build --resume` / `--fresh` flags + crash-gap fix. Build already implicitly auto-resumes off active-build.json, but resume is non-deterministic and the main loop try (build.js:1006) has no catch — a mid-build crash leaves active-build.json stuck at status:'running' with no terminal record, blocking clean recovery. Add: (1) a pure decideBuildStart({active,opts,pidAlive,flowTerminal,sameMode}) helper returning resume\|fresh\|refuse\|error, table-tested; (2) CLI --resume (explicit, errors if nothing resumable) and --fresh (force clean restart, still refuses a live build) mirroring the compose fix/plan resume wiring; (3) a catch that terminalizes a thrown build to status:'failed'+failureReason (flowId preserved), feature.json->PLANNED, build-history append — so crashed builds are uniformly auto-resumable. Reuses the existing opts.resumeFlowId path + startFresh; no new resume mechanics. Single-feature build only (not GSD/checkpoint/--all). Codex-implemented, Claude-reviewed. | COMPLETE |
...
| # | Feature | Description | Status |
|---|---------|-------------|--------|
| 1 | COMP-ROADMAP-RT | Harden deterministic roadmap roundtripping: prove gen↔parse fixed-point + losslessness, write-time auto-canonicalize-or-block guard, unified feature-code parsing, and hierarchy/drift validation findings. | COMPLETE |
...
| 4 | COMP-ROADMAP-GRAPH-1-3 | Provenance/history mode for the roadmap dependency graph — opt-in flag (e.g. --include-completed) that retains COMPLETE nodes (and the edges terminating on them) so a still-active feature's dependency on a now-completed feature stays visible, instead of being silently dropped. | PLANNED |
...
| — | COMP-MIGRATE-UNIFY-VISION | Fold the inline vision-state migrations (legacy featureCode→lifecycle.featureCode + gate-outcome normalize) into the shared state-migration registry as a single implementation reused by both the server load-time paths and the eager runStateMigrations runner. One stateVersion stamp now covers feature.json + vision-state; the eager runner walks cold vision-state too. Consolidation, not a correctness gap (vision-state already migrated on load). | COMPLETE |
...
| 5 | COMP-AGENT-VENDOR-1 | Ship the compose-explorer/compose-architect subagents the SKILL.md depends on — author the two agent definitions and have `compose setup` install them to ~/.claude/agents/ (or switch the skill to built-in Explore/Plan). Today the skill's "Vendored" table references them but no definition file exists anywhere and syncSkills() never installs them, so any `subagent_type: compose-explorer` dispatch fails and the lifecycle silently falls back to built-ins. | COMPLETE |
...
| 7 | COMP-COCKPIT-11 | ChallengeModal "Discuss" posts to /api/terminal/inject (ChallengeModal.jsx:38 via agentServerUrl) but no such route exists on ANY server — dead endpoint, 404s on localhost too (client-side counterpart of the COMP-COCKPIT-10 orphaned-route sweep). Found by the COMP-MOBILE-REMOTE review gate (flagged as a missing remote proxy; root cause is the endpoint never existing). Fix: wire a real inject path on the agent server + proxy it, or remove/disable the Discuss action. | COMPLETE |

---

## COMP-ROADMAP-GRAPH: Dependency Graph — IN_PROGRESS
...
| 2 | COMP-ROADMAP-GRAPH-3 | Render external/cross-product nodes in the roadmap dependency graph so it can serve as the live cross-product dashboard (the surface that replaces foreign ROADMAP.md rows under COMP-ROADMAP-MIGRATE option C). Today the vision adapter drops external-prefixed nodes (lib/roadmap-graph/vision-adapter.js:85) — they're known-but-not-rendered. Add an opt-in mode that renders external/owned-elsewhere codes as reference nodes (resolving status read-only from the owning repo's feature.json), so forge-top's graph shows the full cross-product picture. Sibling of COMP-ROADMAP-GRAPH-2. Anchor: docs/plans/2026-06-21-roadmap-planning-model-design.md. | PLANNED |

---

## COMP-ROADMAP-MODEL: Structured + Composable Roadmaps — PLANNED

| # | Feature | Description | Status |
|---|---------|-------------|--------|
| 1 | COMP-ROADMAP-META | DEFERRED (not v1). NARROW coordination `.compose/roadmap.json` — workspaces[], sharedMilestones[], integrationItems[] only. Augmentation that no feature.json owns; never repeats per-feature data and is NOT a second tracker (Codex round-1/2: don't put themes/GTM/program-tracking/incubation here — ideation stays in the ideabox, and narrative prose is preserved via COMP-ROADMAP-MIGRATE's preserved-sections/docs, not relocated into roadmap.json). Not a prerequisite for MIGRATE (option C needs no roadmap.json). Build only once a real coordination consumer exists. Anchor: docs/plans/2026-06-21-roadmap-planning-model-design.md. | PLANNED |
| 2 | COMP-ROADMAP-COMPOSE | Composable / nested roadmaps. Each sub-project folder is a self-contained compose workspace (own roadmap.json + feature.json set + ROADMAP.md), runnable standalone. The top-level roadmap holds ONLY integration items + links to sub-project roadmaps (composition, no feature repetition). Gaps found: (1) xref local-provider `resolveSiblingRoot` is SIBLING-only — cannot link a top roadmap to a CHILD sub-folder (e.g. forge/ → forge/compose/); parent→child composition unsupported. (2) No cross-workspace composed view / status rollup (each workspace's roadmap + graph is standalone). (3) No 'integration-only' policy/enforcement for a top roadmap (prose-only in the Roadmap Model). (4) Nested parent+child workspaces force explicit --workspace (WorkspaceAmbiguous by design). Depends on COMP-ROADMAP-META (links live in roadmap.json). | PLANNED |
| 3 | COMP-ROADMAP-MODES | KEYSTONE: mode-generalize the lifecycle data so `plan` can be a third lifecycle peer to build/fix. Today transitions/gate-policy/artifact-maps/phase→status are hard-coded to the build graph (lifecycle-guard.js:31, vision-server.js:42, artifact-manager.js:12) and the runner only branches feature-vs-bug (build.js:629,4632). Lift these into mode-scoped data; consolidate the split phase vocabularies (VisionStore.phase vs lifecycle.currentPhase vs feature-scan inference) onto lifecycle.currentPhase. This generalization is also what makes the integration ports pluggable. Unblocks COMP-ROADMAP-PLAN. Anchor: docs/plans/2026-06-21-roadmap-planning-model-design.md. | COMPLETE |
| 4 | COMP-ROADMAP-PLAN | The `plan` product-planning lifecycle v1, peer to build/fix, WITHIN compose (extractable later). Front half of goal-to-product: framing → research → product-level ideation → interactive discussion → estimation/prioritization → convergence to a build-ready spec, ending in a build-handshake artifact (promoted feature.json + approved spec docs) that `build` resumes from. v1 is native-state-only: ideabox funnel + vision-store types idea/thread/question/decision; NO provider-coupled steps (no Linear/JIRA import, no Notion push) in v1. Absorbs the existing `compose new` kickoff (lib/new.js + pipelines/new.stratum.yaml) — stop maintaining a separate kickoff system. Depends on COMP-ROADMAP-MODES. Anchor: docs/plans/2026-06-21-roadmap-planning-model-design.md. | COMPLETE |
| 5 | COMP-ROADMAP-MIGRATE | Migrate forge-top narrative→structured (precondition for retiring narrative). One-time standalone script. Foreign-owned (compose/stratum) rows leave the live ROADMAP.md (option C — ROADMAP.md becomes forge-OWNED-only; foreign codes declared via externalPrefixes so validate/roundtrip skip them); the live cross-product view is the roadmap dependency graph (COMP-ROADMAP-GRAPH-3). Owned-here rows get a real feature.json. Curated changelog/Roadmap-Model prose extracted to docs/preserved-sections (never dropped). Order: classify → scaffold owned-here + set externalPrefixes → preserve prose → dry-run checkRoundtrip → flip narrative:false → validate clean. NOT link-carriers (option B dropped: xref-sync reconciles links[].expect not top-level status, so they'd render placeholders). Depends on COMP-ROADMAP-GRAPH-3. Design: docs/features/COMP-ROADMAP-MIGRATE/design.md. | PLANNED |
| 6 | COMP-ROADMAP-RETIRE | Prohibit narrative roadmap mode — the structured-only end-state. Delete isNarrativeOwned/narrativeOwnedMessage and the 6 narrative gates (feature-writer.js:102, roadmap-gen.js:214/519, bin/compose.js:1164/1221, feature-validator.js:278 NARRATIVE_SUPPRESSED_KINDS). Clean removal (no prod users); structured is already the default for every non-narrative workspace, so removing the gates falls through to it — nothing new to build. MUST land AFTER COMP-ROADMAP-MIGRATE has moved forge-top (the sole narrative workspace) to structured and demonstrated validate-clean parity, else its ROADMAP breaks. Depends on COMP-ROADMAP-MIGRATE. Anchor: docs/plans/2026-06-21-roadmap-planning-model-design.md. | PLANNED |
| 7 | COMP-ROADMAP-PROVIDERS | Integration ports & adapters (post-v1) — keep integrations modular at the domain boundary. TrackerProvider v2: generalize the existing tracker layer (lib/tracker/provider.js, today github-first-class) for Linear/JIRA/GitHub Issues — broaden capability fallback beyond JOURNAL/VISION, tolerate providers that can't natively own ROADMAP.md/CHANGELOG. New seams: DocumentPort (Notion/Confluence for feature docs), DesignPort (Figma/Canva), BuildTargetPort (Base44/Webflow) — none exist today (lifecycle writes local .md, artifacts[] is a local sidecar). Canon stays NATIVE + integration-agnostic; providers are opt-in adapters at REFERENCE/DRIVE/SYNC/SUBSUME depth, never imported by the core lifecycle. v1 ships REFERENCE-only attach (the xref enum reserves jira\|linear\|notion but they're inert pointers today, feature-validator.js:1044). Anchor: docs/plans/2026-06-21-roadmap-planning-model-design.md. | PLANNED |
| 8 | COMP-ROADMAP-EPIC | First-class epic/umbrella entity. Today an epic is a free-text phase string on each feature.json PLUS a status override smuggled into the ROADMAP.md heading (roadmap-gen.js:62); the override fights the computed rollup (phaseStatus, :33) and emits perpetual drift WARNs. Provenance (anchor doc, north-star, ordered constituents) lives in prose. The 'track' vision type (vision-store.js:10) exists but is stranded/unused. Promote the epic to a tracked node (likely by wiring 'track'): code+description+status(rollup-derived, authored-override as an entity field)+anchorDoc+northStar+ordered constituents; constituents ref epic by code (retiring the phase string); retire the heading-override hack and its drift path. Sibling of COMP-ROADMAP-COMPOSE. Anchor: docs/plans/2026-06-21-roadmap-planning-model-design.md. | PLANNED |

---

## Phase 11: Experimentation — IN_PROGRESS
...
| # | Feature | Description | Status |
|---|---------|-------------|--------|
| 1 | COMP-STRATUM-TS | Flag-gated cutover of compose's stratum flow/gate monitor seam to the TS engine (STRAT-TS-PORT P7): engine-selectable stratum-client; guard stays on Python (STRAT-GUARD not ported); agent-authoring cutover deferred to COMP-STRATUM-TS-2 | COMPLETE |
...
## COMP-AUDIT: 2026-07 Cockpit Wiring, Completeness & UX Audit — PARTIAL

| # | Feature | Description | Status |
|---|---------|-------------|--------|
| 1 | COMP-AUDIT-1 | Fix ContractEditor infinite re-render crash: Pipeline Editor → step → Contracts throws "Maximum update depth exceeded" (uncached useSyncExternalStore snapshot returns a fresh ref each call). Reproducible 100%; recoverable only via PanelErrorBoundary retry. | COMPLETE |
| 2 | COMP-AUDIT-2 | Governance Settings modal fake-saves: SettingsModal.jsx writes settings.governance.phases but settings-store.js:134 rejects `governance` as an unknown top-level key, and the modal flashes 'Saved' without awaiting the result. Governance toggles never persist. | COMPLETE |
| 3 | COMP-AUDIT-3 | Gate surface trust bugs: nav 'GATES' badge counts resolved+pending (shows 9 when 1 is actually pending); the pending gate plus one historical gate render the phase transition as 'null -> null'. Undermines the approval surface the product is built around. | COMPLETE |
| 4 | COMP-AUDIT-4 | 'Pressure Test' button in the item detail panel is a no-op (ContextItemDetail.jsx:106 onPressureTest={() => {}}). Wire it to a real endpoint or remove the control. | COMPLETE |
| 5 | COMP-AUDIT-5 | Mobile agent list never clears finished/killed agents: useLiveAgents.js:61 listens for agentSpawned plus 3 never-emitted names while the server emits agentComplete/agentKilled. A done or killed agent shows as running forever on mobile. | COMPLETE |
| 6 | COMP-AUDIT-6 | Graph gate popover Kill resolves with no reason (GraphView.jsx:544) — one-click irreversible kill with no rationale, bypassing the kill-reason guard. Contradicts COMP-COCKPIT-6 (COMPLETE). | COMPLETE |
| 7 | COMP-AUDIT-7 | Ten build-stream events (qa_scope, step_usage, gate_tier_result/failed/summary, test_review, health_score, capability_violation, idea_suggestion, buildStreamEvent) are broadcast to the agent SSE stream but dropped by the desktop renderer (build-stream-bridge -> MessageCard.jsx:318). Route them to a canonical consumer: this alone revives the dead QA Scope view and surfaces build cost/health/test telemetry. | COMPLETE |
| 8 | COMP-AUDIT-8 | Empty-cockpit 'create your first feature' CTA opens the generic vision-item dialog (App.jsx:933), not the scaffold flow (NewFeatureDialog.jsx) — the created 'feature' has no feature.json/design/ROADMAP entry and cannot enter `compose build`. Contradicts COMP-COCKPIT-5 (COMPLETE). Worst flow to have broken: it is the first one a new user touches. | PLANNED |
| 9 | COMP-AUDIT-9 | Cockpit completion marks the vision item complete (vision-routes.js:505) but without a commit SHA only projects to feature.json in guarded mode — a cockpit-completed feature can show done in the UI while feature.json/ROADMAP stay stale. | PLANNED |
| 10 | COMP-AUDIT-10 | `compose new` (lib/new.js, new.stratum.yaml) still ships alongside `compose plan` despite COMP-ROADMAP-PLAN saying to retire the old kickoff lifecycle — two goal-to-feature journeys with different artifacts and behavior. Retire or reconcile. | PLANNED |
| 11 | COMP-AUDIT-11 | Split phase model: VisionStore keeps a legacy phase enum (vision-store.js:13) and the cockpit filters on item.phase (App.jsx:689) while COMP-ROADMAP-MODES promised consolidation on lifecycle.currentPhase — lifecycle phase and UI filter state can disagree. Collapse during the stratum TS cutover rather than porting both. | PLANNED |
| 12 | COMP-AUDIT-12 | Persisted settings with no runtime effect (models.agentRun, models.summarizer, health.enabled, thinking) and decorative/never-written fields (assignedTo, item-level governance, session commits[], semanticId). Wire each to real behavior or remove so saved config is not a lie. | PLANNED |
| 13 | COMP-AUDIT-13 | docs/cli.md and `compose --help` omit shipped subcommands (plan, gsd, validate, experiment, remote, tracker, items, context, smartmemory) — installed capability is invisible unless users read source. Regenerate the reference and help from the real command table. | PLANNED |
| 14 | COMP-AUDIT-14 | docs/mcp.md omits 9 registered MCP tools (set_workspace, get_workspace, get_roadmap, validate_feature, validate_project, roadmap_graph, roadmap_graph_check, roadmap_xref_push, propose_followup) — MCP clients and agents cannot discover them. | PLANNED |
| 15 | COMP-AUDIT-15 | ~20 uncalled server routes (Canvas subsystem + Canvas.jsx unreachable from App.jsx, 5 lifecycle/artifact routes, feature scan/seed, stratum bind/audit, agent gc/list, duplicate settings/summary reads) plus the legacy vision-track CLI defaulting to port 3001 while the server runs on 4001. Delete or consciously keep before the TS port carries the dead surface forward. | PLANNED |
| 16 | COMP-AUDIT-16 | Dead WS protocol: desktop vision handler retains never-emitted branches (decisionEventsSnapshot, visionUI, gatePending); mobile keeps 10 legacy handlers; lifecycleReconciled is emitted but unhandled; desktop interrupt failures are swallowed with no user feedback. Prune the dead branches and surface interrupt errors. | PLANNED |
| 17 | COMP-AUDIT-17 | Information architecture: 14 flat top-level views plus a ~10-icon unlabeled toolbar, with overlapping Dashboard/Graph/Items and confusing Pipeline vs Pipeline Editor. Group into Work / Review / Build / Knowledge and label or drop the icon rail and the sidebar double-number group counts. | PLANNED |
| 18 | COMP-AUDIT-18 | Land on Dashboard (and fatten it) instead of the Graph, which is unreadable at default zoom (use a fit-to-readable initial zoom); remove or repurpose the permanently-inert 'Select a feature to see status.' global bar shown atop every view; and resolve the three competing status models on one item (status dropdown vs lifecycle phase vs confidence) so it is clear which governs `compose build` (see COMP-AUDIT-11). | PLANNED |
| 19 | COMP-AUDIT-19 | Roadmap-writer MCP tools return the full rendered ROADMAP (~190KB) in the tool result, exceeding the MCP token cap on EVERY call — add_roadmap_entry blew 193,821 chars while filing COMP-AUDIT-1 this session; per reference notes set_feature_status/propose_followup/roadmap generate\|status likely do too. addRoadmapEntry's declared return is small ({code, roadmap_path:<path string>} — writeRoadmap returns a path, not content), so the full markdown is injected somewhere in the MCP result path (compose-mcp.js dispatch / result wrapper). The blessed 'use this instead of editing ROADMAP.md by hand' surface is thus unusable at any batch scale and forces direct feature.json + `roadmap generate` workarounds (used for AUDIT-1..18). Fix: writers return only {code, roadmap_path, row} (never full markdown; cap any preview) and emit a small success signal so callers can tell success from a serialization overflow. Secondary: the roundtrip guard runs BEFORE the feature.json write, so concurrent fan-out can drop a feature — document as sequential-only or make write+guard atomic. Complements COMP-ROADMAP-SHARD (that shrinks the FILE; this shrinks the TOOL RETURN). | COMPLETE |

---

## COMP-PLAN-RIGOR: Front-of-Funnel Rigor + Parity — PLANNED

| # | Feature | Description | Status |
|---|---------|-------------|--------|
| 1 | COMP-PLAN-IDEA-UNIFY | Unify ideation storage behind the fluid-store provider seam (owner ruling 2026-07-21, PROVIDER-SEAM in what-to-build 8k): ideabox records become provider records on a zero-install local floor implemented over vision idea-type items; SmartMemory is the capability-rich reference provider backing the same records later. ideabox.md becomes a read-only generated view (ROADMAP.md<-feature.json pattern). Ideabox stays the funnel/UI/CLI. Pilot workload of the provider seam positions/joints/decisions adopt next. Honors COMP-ROADMAP Decision 4 (funnel interface pinned, not storage). One-time markdown->store migration; no bidirectional sync. Rung 1 (keystone) of COMP-PLAN-RIGOR. Anchor: docs/design/2026-07-20-front-funnel-rigor-design.md; ruling: docs/product/2026-07-20-what-to-build-vision.md 8k. | PLANNED |
...
## Judgment Layer — PLANNED

| # | Feature | Description | Status |
|---|---------|-------------|--------|
| 1 | COMP-CANON-GUARD | **The write-guard for canonical artifacts — make TOOLS-OWN-WRITES and BLOCK-THE-BYPASS real.** Phase-1 review overturned the premise: a guard already EXISTS and is switched off. `lib/mcp-enforcement.js` declares guarded files + authorised typed tools and `lib/build.js:3963` runs a correlated pre-stage scan at ship — but `.compose/data/settings.json` is `{}`, so the mode resolves to `off`. UNGUARDED-WRITES survives; its diagnosis is amended (enforcement that ships disabled is the purest form of routable-around). Four real gaps: G1 it is disabled (a settings key); G2 it does not cover `docs/judgment/**`, where the worst failures happened; G3 it enforces at ship time only, so a whole session can go wrong before commit; G4 it verifies THAT a tool was used, not WHO wrote it — provenance is the genuinely new part, and the 2026-07-21 agent-claim-as-owner-claim failure is what it makes unrepresentable. Adds one `canon-registry.js` consumed by BOTH the existing ship-time scan and a new PreToolUse hook (complementary: the hook prevents, the scan backstops and already covers Bash/sed/git writes the hook cannot see). Registration is gated on an operation inventory — no tool can currently update a feature description/phase/tags, and roadmap prose needs a preserved-section bootstrap. Override is grant-then-write, ledger-first, single-use and path-scoped, per OVERRIDES-ARE-GOLD. | COMPLETE |
| 2 | COMP-JUDGMENT-WRITER | Typed writer + provider seam for judgment canon (COMP-CANON-GUARD S3): git-tracked records under docs/judgment/records/, markdown as fixed-point projections, 4 owner-locked judgment_* tools + read tool, v1 agent-only provenance, SmartMemory as enrichment layer (fast-follow) + two-part ontology RFC. Anchor: docs/features/COMP-JUDGMENT-WRITER/design.md | COMPLETE |
| 3 | COMP-JUDGMENT-STORES | Writer extensions — person/situation/goal/resolution-package record kinds, generated projections, invariants as rejected writes (the only door for every store; manual Writer box + ledger seq 112) | COMPLETE |
| 4 | COMP-JUDGMENT-PACKAGES | Resolution packages + transition coupling — seal-via-transition, joint-state×op matrix, intent fencing, method binding, prediction lifecycle (open→graded\|void). Split from COMP-JUDGMENT-STORES at gate round 5 (the finding-dense corner gets its own gate) | PLANNED |
| 5 | COMP-JUDGMENT-GOAL-MIGRATE | Intent-backed objective→goal migration: replay-durable compound op, dual-read cutover, intent-aware snapshot, position retirement. Split from COMP-JUDGMENT-STORES at gate round 5; depends on the goal store shipping first | COMPLETE |
| 6 | COMP-CANON-ATTEST | Content attestation for guarded canon (COMP-CANON-GUARD S6): typed writers record a hash of what they produced; the ship-time scan compares every dirty guarded path against its last attested hash, so an interleaved tool-call-plus-hand-edit is detected. Until this lands the Bash bypass is a known, documented gap — S4's PreToolUse hook cannot see Bash/sed/git writes and S5's scan verifies THAT a tool ran, not WHAT it wrote. | PLANNED |
| 7 | COMP-BUILD-CORRELATION | Repair build-correlation on feature events so enforcement can ever be promoted to 'block'. COMP-CANON-GUARD phase-1 forensics found all 777 real events across 2.5 months carry build_id: null (feature-events.js emits null outside a build runner; scanGuarded only considers events matching the current buildId) — so the ship-time correlated scan has never been satisfiable and S2b was retired on this finding. Fix the correlation model (or drop correlation as the matching key) before any 'block' promotion is re-specced. | PLANNED |
| 8 | COMP-CANON-INVENTORY | Full canon write-surface inventory (COMP-CANON-GUARD S2): every path that mutates canon outside a typed tool — roadmap-gen, feature-writer, migrate-anon, experiment, roadmap-graph, bin/compose.js (feature create + roadmap generate + init template), server/ideabox-routes — is routed through a typed tool, retired, or declared override_only with a reason, verified at registration. Adds the missing update_feature_fields tool (description/phase/tags/profile on an existing feature). NOTE: this no longer gates a 'block' promotion — S2b was retired on the build_id: null finding, so the payoff is hygiene until COMP-BUILD-CORRELATION lands. | PLANNED |
| 9 | COMP-CANON-OVERRIDE | The canon override — grant-then-write, ledger-first (COMP-CANON-GUARD Decision 4, deferred at S4 and never built). canon_override_grant({path, reason, operation}) MCP tool: rejects empty/whitespace reason, writes the ledger entry BEFORE minting, token is single-use and path-scoped so the override costs something visible rather than decaying into 'turn the guard off'. Blocks COMP-CANON-ATTEST, which needs a governed reconcile path for a chain break. Must also answer the ledger circularity: an authorization written into ledger.jsonl breaks the very chain it authorizes, so ledger-drift recovery needs a bootstrap route outside the attested set. | PLANNED |
...
| # | Feature | Description | Status |
|---|---------|-------------|--------|
| 1 | COMP-UPDATE-NUDGE | **Tell the user they're behind — never update in place.** `compose update` already does the whole job (install-style detection, reinstall/ff-pull, skill refresh, state migration, `healStratumWiring`, MCP-restart warning), but nothing tells you to run it: you learn you're behind only if you happen to run `compose doctor`. A second gap opened when compose 0.3.7 moved stratum from an exact pin to `^0.3.3` — npm won't re-resolve a caret unless something triggers an install, so "compose is current" no longer implies "stratum is current" and a compose-only check calls that state healthy. This slice generalizes `lib/version-check.js` to any package, reads the RESOLVED stratum version from `node_modules` (not the declared range — the range is what drifts), and prints ONE line at three entry points (`init`, `doctor`, `build`/`plan`) naming whichever is behind. Silent when current or on any failure. Deliberately never self-updates: `bin/compose.js` + `lib/**` hold 109 lazy `await import()` sites, so an in-place npm reinstall over a running process lazily loads NEW modules into an OLD graph — mixed-version code in one process, with the long-lived MCP server most exposed. Safe auto-update needs a versioned install (COMP-UPDATE-VERSIONED-INSTALL), not a timing guardrail. | PLANNED |
| 2 | COMP-UPDATE-VERSIONED-INSTALL | **Make auto-update safe by construction, not by timing.** Compose cannot update itself in place: `bin/compose.js` + `lib/**` hold 109 lazy `await import()` sites, so an `npm install -g @latest` over a running process leaves it importing NEW modules into an OLD module graph — mixed-version code inside one process, worse than stale code and undebuggable from outside, with the long-lived MCP server most exposed. No timing guardrail fixes it, because a CLI invocation cannot know what else is running. The approach worth copying is Claude Code's native install (verified 2026-07-25): versions side by side under `versions/<v>` with a symlink flip to activate, last 3 retained, so nothing a running process depends on is ever overwritten and the next launch picks up the new build. Adopting it means compose stops being a plain npm global: version-directory layout, launcher shim, migration for every existing npm/git install, retention + rollback policy, and version-scoped paths interacting with `healStratumWiring`'s `.mcp.json` entries. Stratum rides along inside the versioned tree, incidentally fixing caret drift. Filed rather than built — COMP-UPDATE-NUDGE ships the notify-only slice first. | PLANNED |
| 3 | COMP-SEMVER-STRICT | **`compareVersions` accepts malformed input and answers confidently.** `compareVersions('1.2.3garbage', '1.2.4')` returns -1 ("behind") instead of null, because the parser uses `Number.parseInt` per component and `parseInt('3garbage')` yields 3 — trailing junk is silently discarded rather than rejected. Reproduced 2026-07-25 during the COMP-UPDATE-NUDGE Task 1 review. Impact today is low: the only inputs are package.json `version` fields and npm registry responses, both well-formed, so the bad path is not currently reachable. It is filed rather than fixed inline because `lib/version-check.js:65-85` is pre-existing shipped code that `compose doctor` and its golden output depend on, and COMP-UPDATE-NUDGE Task 1 was scoped to plumbing — widening it to touch the comparator would have put doctor's output in the blast radius of an unrelated task. Fix: reject any component with non-numeric trailing characters (return null, matching the existing contract for unparseable input), with tests for `1.2.3garbage`, `1.2.x`, `v1.2.3`, and empty components, and a check that doctor's output is unchanged. | PLANNED |
