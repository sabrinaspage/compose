 ...
 * CLI and python `stratum-mcp` both emit and no incompatible/adversarial binary
 * reproduces. Returns { ok, reason? }; never throws.
 *
 * @param {string} bin
...
export function probeStratumBin(bin, deps = {}) {
  ...
  // message must equal the exact projection format the real CLIs emit (verified:
  // both the TS CLI and python `stratum-mcp` emit `Flow '<id>' not found`).
...
/** Resolve env -> project capability -> flag-day TS default. */
...
export function resolveStratumMcpConnection(cwd) {
  if (resolveStratumEngine(cwd) === 'python') {
    return { command: 'stratum-mcp', args: [], ...(cwd ? { cwd } : {}) };
  }
  return {
