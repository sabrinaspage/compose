...
export class StratumMcpClient {
  ...
  async connect(opts = {}) {
    ...
    try {
      ...
      // Tool discovery is advisory for compatibility guards. Servers that do
      // not support it retain the historical call-through behavior.
  ...
  async plan(spec, flow, inputs) {
    ...
    return this.#callTool('stratum_plan', { spec: parsedSpec, input: inputs });
  }
   ...
   * @param {number} [epoch] - Engine-issued ready-entry epoch to echo (fencing)
   * @param {string} [dispatchToken] - Engine-issued ready-entry issuance token to echo
   * @returns {Promise<object>}
   */
  async stepDone(flowId, stepId, result, epoch, dispatchToken) {
    return this.#callTool('stratum_step_done', {
      runId: flowId,
      stepId,
      result,
      // Phase-1 fencing (STRAT-TS-FANOUT-CONSUMER amendment): echo the engine-issued
      // ready-entry epoch so a stale report is rejected instead of silently accepted.
      ...(Number.isInteger(epoch) ? { epoch } : {}),
      // Phase-2 universal fencing: tokens are opaque and only non-empty strings
      // are transmitted, preserving every pre-token caller.
  ...
  #assertPythonParallelLifecycle(toolName) {
    // Pre-connect calls fall through to #callTool's own not-connected error.
    if (!this.#connected) return;
    if (this.#availableTools === null) {
      // Fail closed: without discovery we cannot tell a python-era surface
      // from the TS one, and calling through would surface as an opaque
      // unknown-tool protocol error on TS servers.
      throw new Error(
        `MCP tool discovery failed at connect — cannot confirm python-era parallel lifecycle support (${toolName})`,
      );
    }
    if (!this.#availableTools.has(toolName)) {
      throw new Error(
        `python-era parallel lifecycle is not supported on the TS path (${toolName})`,
      );
    }
  }
  ...
  async parallelDone(flowId, stepId, taskResults, mergeStatus) {
    this.#assertPythonParallelLifecycle('stratum_parallel_done');
  ...
  async parallelStart(flowId, stepId) {
    this.#assertPythonParallelLifecycle('stratum_parallel_start');
  ...
  async parallelPoll(flowId, stepId) {
    this.#assertPythonParallelLifecycle('stratum_parallel_poll');
  ...
  async parallelAdvance(flowId, stepId, mergeStatus) {
    this.#assertPythonParallelLifecycle('stratum_parallel_advance');
