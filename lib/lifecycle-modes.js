...
export const LIFECYCLE_MODES = {
  ...
  // -------------------------------------------------------------------------
  // JUDGMENT — the joint state machine (COMP-JUDGMENT-WRITER design Decision 2).
  // Consumed ONLY by lib/judgment-writer.js through guardedTransition(mode:
  // 'judgment'), which makes the Stratum guard the single lifecycle authority
  // for joint transitions where capabilities.guard is on. The per-edge required
  // ARTIFACTS (resolution packages, dissolution, reopen, re-dispose) are
  // enforced by lib/judgment-write-guard.js before the guard is called —
  // edgeEvidence stays empty because the evidence lives inside records, not in
  // per-feature files. Never run by any CLI runner; the runner block is inert.
  //
  // KNOWN SUPERSET: buildPhaseGraph() unconditionally adds `resolved →
  // complete` and `<non-terminal> → killed` edges to every registered graph
  // (an adapter invariant this feature may not touch). Those five extra edges
  // are UNREACHABLE through the judgment writer — the write-guard's edge→
  // artifact table refuses them before any guard call — and the parity test
  // pins the superset to exactly that auto-added set. Closing them at the
  // guard itself is epic S4–S6 enforcement territory.
  // -------------------------------------------------------------------------
  judgment: {
    transitions: {
      open: ['under_test', 'superseded', 'dissolved'],
      under_test: ['resolved', 'inconclusive', 'open', 'superseded', 'dissolved'],
      resolved: ['open'],
    ...
    phaseArtifacts: [],
    edgeEvidence: {},
    phaseOrder: ['open', 'under_test', 'resolved', 'inconclusive'],
    runner: {
      artifactRoot: 'docs/judgment/records/joints',
      runsTriage: false,
      tracksFeatureJson: false,
      descriptionLoader: 'plan',
      planInputs: 'plan',
