...
/**
 * Current baked versions for hooks whose content contract is versioned.
 * Post-commit remains unversioned until its template changes.
 */
...
/** Pull the baked HOOK_VERSION="..." value out of hook content, or null. */
export function extractBakedHookVersion(content) {
  const m = content.match(/^HOOK_VERSION="([^"]*)"$/m);
  return m ? m[1] : null;
}
...
function computeOne({ type, hooksDir, expectedWsId, composeNode, composeBin }) {
  ...
  const expectedVersion = HOOK_VERSIONS[type] ?? null;
  ...
  const bakedVersion = extractBakedHookVersion(content);
  const versionMatch = expectedVersion == null || bakedVersion === expectedVersion;
  ...
  if (nodeMatch && binMatch && wsMatch && !hasRawToken && versionMatch) {
...
export function formatHookStatusLines(type, s, { composeNode, composeBin }) {
  ...
  if (s.reason === 'HOOK_VERSION_DRIFT') lines.push(`  expected HOOK_VERSION="${s.expectedVersion}"`);
 ...
 * @property {null|'MISSING_WORKSPACE_ID'|'STALE_WORKSPACE_ID'|'stale paths'|'HOOK_VERSION_DRIFT'} reason
 * @property {string|null} bakedWorkspace
 * @property {string|null} bakedVersion
