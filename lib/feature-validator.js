...
function runArtifactLinkChecks(fctx, ctx, findings) {
  ...
  // MISSING_COMPLETION_REPORT (info, COMPLETE)
  // The report phase is OPTIONAL/skippable (small features legitimately never
  // write one), so a missing report.md is informational, not warning-level debt.
  ...
  // ship — those stay fully exempt (no finding at all).
  if (status === 'COMPLETE' && !folder.files.has('report.md') &&
      featureJson?.built_via !== 'build-quick') {
    findings.push(finding('info', 'MISSING_COMPLETION_REPORT', code,
      `Feature is COMPLETE without report.md — the report phase is optional (skippable for small features); informational only`));
  }
