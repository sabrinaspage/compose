 ...
 *   - injectVocabularyEnsure()   : append the vocab ensure to a v0.3 build flow's `review` step (no-op for v1 — see F5)
...
export function injectVocabularyEnsure(specObj, flowName) {
  // E3/F5: v1 vocabulary enforcement is deterministic + compose-side (evaluated
  // at review_merge; see computeVocabularyStepFailure in lib/build.js). The old
  // TS judged ensure could never see the changed files or the vocabulary, so it
  // is NOT injected — a v1 spec gets no vocab ensure. Only the v0.3 python path
  // (evaluable vocabulary_compliance builtin) keeps its injected ensure.
  if (specObj?.version === 1) return specObj;
  ...
  const key = JSON.stringify(VOCABULARY_ENSURE);
  if (!review.ensure.some((entry) => JSON.stringify(entry) === key)) {
    review.ensure.push(structuredClone(VOCABULARY_ENSURE));
  }
  return specObj;
}
