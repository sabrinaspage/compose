...
import { appendEvent as appendDispatchEvent } from './dispatch-ledger.js';
...
export function createBuildAccumulator(projectCwd, featureCode) {
  ...
  return writeBuildAccumulator(projectCwd, accumulator);
}
...
export function selectBuildAccumulator(projectCwd, featureCode, { fresh = false } = {}) {
  ...
  if (fresh || !existing || ['complete', 'aborted'].includes(existing.last_terminal)) {
    return { accumulator: createBuildAccumulator(projectCwd, featureCode), isNew: true };
  }
...
export function emitBuildActuals(projectCwd, accumulator, terminalStatus) {
  ...
  let finalAccumulator = persisted;
  if (terminalStatus === 'failed') {
    finalAccumulator = updateBuildAccumulator(projectCwd, persisted.feature_code, (current) => ({
      ...current,
      last_terminal: 'failed',
    }));
  }
  const authoritativeShip = Array.isArray(finalAccumulator.ship_files_changed);
  const files = authoritativeShip
    ? finalAccumulator.ship_files_changed
    : finalAccumulator.files_changed;
  const row = appendDispatchEvent(projectCwd, {
    kind: 'build-actuals',
    build_id: finalAccumulator.build_id,
    feature_code: finalAccumulator.feature_code,
    terminal_status: terminalStatus,
    files_changed_count: new Set(files).size,
    files_source: authoritativeShip ? 'ship' : 'accumulated',
    review_iterations: finalAccumulator.review_iterations,
    escalations: finalAccumulator.escalations,
    tokens_total: finalAccumulator.tokens_total,
    usd: finalAccumulator.usd,
    test_count: finalAccumulator.test_count,
    pass_rate: finalAccumulator.pass_rate,
  });
  if (terminalStatus === 'complete' || terminalStatus === 'aborted') {
    clearBuildAccumulator(projectCwd, finalAccumulator.feature_code);
  }
  return row;
}
...
export async function runBuild(featureCode, opts = {}) {
  ...
  const finalizeBuildAttempt = () => {
    ...
    const terminalStatus = buildStatus === 'complete'
      ...
      : (buildStatus === 'killed' || buildStatus === 'aborted')
        ? 'aborted'
        : 'failed';
    const accumulator = readBuildAccumulator(cwd, featureCode);
    ...
    // guard; the missing-file guard keeps cleanup idempotent.
    if (accumulator) emitBuildActuals(cwd, accumulator, terminalStatus);
  ...
  if (isNewAccumulator && triageEstimate) {
  ...
  try {
    ...
    // A fresh start must not inherit a failed attempt's identity: the failed
    // accumulator's terminal row already closed that build_id's story, and a
    // reused id would suppress this attempt's estimate. Rotate to a new
    // accumulator (new build_id + its own estimate row).
    if (isFreshStart && !isNewAccumulator) {
      const rotated = createBuildAccumulator(cwd, featureCode);
      build_id = rotated.build_id;
      process.env.COMPOSE_BUILD_ID = build_id;
      if (triageEstimate) {
    ...
    const context = {
      ...
      implementerAgent,
      reviewerAgent,
      filesChanged: [...selectedAccumulator.files_changed],
