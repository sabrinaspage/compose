...
import { vocabularyEnabled, injectVocabularyEnsure, tagVocabularyViolations, VOCABULARY_FILE } from './vocabulary-inject.js';
 ...
 */
export function deriveOrdinaryReviewScaffold({ response = {}, contractName = null, stepId = '', reduceSteps } = {}) {
...
export async function runConsumerIssuance({
  ...
  // reasoning template — lived in compose's buildReviewPrompt wrapper on the legacy
  // parallel path (not the yaml). Apply the same wrapper here so consumer review
  ...
  progress.stepStart('?', '?', descriptor.id);
  streamWriter.write({
  ...
  artifacts.prepareIssuance(descriptor, envelope, {
    finalStage,
  });
...
function resolveConsumerConcurrency(descriptors = []) {
  ...
  // future engine policy field if one appears; until then retain Python parity
  // (3) with an explicit Compose-side override.
...
// to every changed test file. Mirrors the prior_dirty_lenses sidecar pattern.
...
function isTerminalFlow(status) {
  return ['completed', 'failed', 'budget_exhausted', 'complete', 'killed'].includes(status);
}
...
function isRecoverableFlowProbeError(err) {
  return err?.code === 'flow_not_found'
    ...
    // Surface 9 reports a missing persisted run through MCP -32603 with the
    // store's ENOENT message rather than the Python flow_not_found envelope.
    || (err?.code === -32603 && /ENOENT: no such file or directory.*\.json/.test(err?.message ?? ''));
}
...
async function terminalizeThrownBuild({
  ...
  const flowId = response?.runId ?? response?.flow_id ?? null;
...
export async function runBuild(featureCode, opts = {}) {
  ...
  try {
    ...
    if (verdict.action === 'resume') {
      ...
      if (!isFreshStart) {
        const stepId = response.ready?.[0]?.id ?? response.step_id;
        const flowId = response.runId ?? response.flow_id ?? resumeFlowId;
    ...
    if (implementerAgent === 'codex') {
      ...
      if (!probe.ok) {
        ...
        try {
          ...
          const sameFlow = !cur?.flowId || !response?.flow_id || cur.flowId === response.flow_id;
    ...
    streamWriter.write({
      ...
      featureCode,
      flowId: response.runId ?? response.flow_id,
      specPath: `pipelines/${templateName}.stratum.yaml`,
    });
    ...
    const context = {
      ...
      stepHistory,
      mode,
      // V4: merged runtime + static agent profiles, so subflow steps dispatched
      // through executeChildFlow (e.g. coverage_check/run_tests) recover their
      // tier/capability profile via resolveStepProfile's scoped-id normalization.
    ...
    while (!isTerminalFlow(response.status)) {
      ...
      if (response.status === 'ready' || response.status === 'execute_step') {
        ...
        // G1/H1: on the TS path the python-era review_mode/reduce_mode inputs are
        // gone. Review-ness comes from the resolved output contract identity (e.g.
        ...
        const resolveGateWithConsumerMerge = async (requestedOutcome, requestedRationale, resolvedBy) => {
          ...
          if (outcome === 'kill' && (next.status === 'failed' || next.status === 'budget_exhausted')) {
            next = { ...next, status: 'killed' };
          }
        ...
        if (policy.mode === 'skip') {
          ...
          // persisted flow file (fresh TS state uses round 0; legacy fail-open is 1).
        ...
        if (childFlowName === 'coverage_check') {
          // COMP-TEST-BOOTSTRAP-4-1: snapshot test files present BEFORE coverage runs,
          // so the later test_review step can review only the tests coverage generates.
          // Persisted to a sidecar so a resume landing on test_review keeps the baseline.
          {
            const preCov = listChangedTestFiles(agentCwd);
            context._preCoverageTestFiles = new Set(preCov);
            persistPreCoverageTests(composeDir, preCov);
          }
        ...
        // COMP-TEST-BOOTSTRAP-4-1: the test_review step reviews the test files the
        // coverage step produced this build (the implementation review runs BEFORE
        // coverage, so generated tests are never otherwise reviewed). Scope to NEW
        // test files (not present before coverage); skip the step entirely when none.
        if (childFlowName === 'test_review') {
          // Resume-safe: fall back to the sidecar when the in-memory snapshot is absent
          // (a build resumed after coverage rebuilds context fresh).
          const preCoverage = context._preCoverageTestFiles ?? loadPreCoverageTests(composeDir);
          const generatedTests = listChangedTestFiles(agentCwd).filter(f => !preCoverage.has(f));
          if (generatedTests.length === 0) {
            progress.subFlowStep('test_review', 'skipped — no generated tests');
            const skipResult = { clean: true, summary: 'No generated test files this build — test review skipped', findings: [] };
            buildSignals.test_review = skipResult;
            streamWriter?.write({ type: 'test_review', clean: true, summary: skipResult.summary, findings: [] });
            verifyPipelineIntegrity(specPath, specFileHash);
            response = await stratum.stepDone(parentFlowId, parentStepId, { status: 'ok', output: skipResult });
            continue;
          }
          // Inject the exact file list into the review intent (child-flow inputs are
          // declarative; intent-append is the established runtime-injection pattern).
          if (response.child_step?.intent !== undefined) {
            response = {
              ...response,
              child_step: {
                ...response.child_step,
                intent: `${response.child_step.intent}\n\nGenerated test files to review (written this build):\n${generatedTests.map(f => `- ${f}`).join('\n')}`,
              },
            };
          }
        }
        ...
        // COMP-CODEX-IMPL: when Codex is the IMPLEMENTER, this Codex second-opinion pass
        // would be Codex reviewing its own diff — suppress it. The Claude parallel_review
        // lenses already provide cross-model review of Codex's implementation.
        if (childFlowName === 'parallel_review' && context.implementerAgent === 'codex') {
          streamWriter?.write({ type: 'cross_model_review', status: 'codex_skipped', reason: 'Codex is the implementer — its own diff is reviewed by Claude lenses, not a Codex second opinion' });
        } else if (childFlowName === 'parallel_review' && childResult?.output && (context.filesChanged?.length ?? 0) > 0) {
        ...
        } else if (childFlowName === 'test_review' && childResult?.output != null) {
          // COMP-TEST-BOOTSTRAP-4-1: advisory — never blocks ship. Surface findings on
          // the build stream (buildSignals alone is not read by any consumer) so the
          // cockpit/report can show them.
          const tr = childResult.output;
          buildSignals.test_review = tr;
          streamWriter?.write({
            type: 'test_review',
            clean: tr.clean ?? true,
            summary: typeof tr.summary === 'string' ? tr.summary : '',
            findings: Array.isArray(tr.findings) ? tr.findings : [],
          });
        }
        ...
        // STRAT-VOCAB-3: surface vocabulary violations as must-fix (they block ship).
        if (violations.length > 0) progress.findings(tagVocabularyViolations(violations));
    ...
    if (response.status === 'failed' || response.status === 'budget_exhausted') {
      buildStatus = 'failed';
      terminalFailureReason = response.failure?.reason ?? null;
    }
    if ((response.status === 'completed' || response.status === 'complete') && buildStatus === 'complete') {
      console.log('\nBuild complete.');
      ...
      await writeFailedBuildTerminalState({
        ...
        itemId,
        featureCode,
        flowId: response?.runId ?? response?.flow_id ?? null,
    ...
    if (response.trace) {
      ...
      try {
        const audit = await stratum.audit(response.runId ?? response.flow_id);
        mkdirSync(featureDir, { recursive: true });
        writeFileSync(
          join(featureDir, 'audit.json'),
...
export async function executeChildFlow(
  ...
  while (resp.status !== 'complete' && resp.status !== 'killed') {
    if (resp.status === 'execute_step') {
      ...
      let childMainResult;
      try {
        childMainResult = await runAndNormalize(null, prompt, resp, { progress, streamWriter, maxDurationMs: childStepTimeout, onToolUse: childOnToolUse, stratum, cwd: context.cwd, profile: resolveStepProfile(context.stepProfiles, resp.step_id) });
      } catch (err) {
        if (err instanceof UserInterruptError) {
          if (err.action === 'skip') {
      ...
      // STRAT-VOCAB-3: surface vocabulary violations as must-fix (they block ship).
      if (violations.length > 0 && progress) progress.findings(tagVocabularyViolations(violations));
      ...
      // COMP-CODEX-IMPL: the fixer is the IMPLEMENTER — whoever wrote the code fixes
      // the review findings against it. This replaces the old hardcoded
      // `stepAgent === 'codex' ? 'claude' : stepAgent` swap, which assumed codex is
      // always the reviewer and claude always the implementer; that assumption inverts
      // under --codex. Default (implementer=claude) reproduces the old outcome exactly:
      // a codex reviewer's finding is fixed by claude, the implementer. (stepAgent is
      // still the reviewer, used for the retry display below.)
      const fixAgent = context.implementerAgent || 'claude';
      ...
      try {
        childRetryResult = await runAndNormalize(null, prompt, resp, { progress, streamWriter, maxDurationMs: fixTimeout, stratum, cwd: context.cwd, profile: resolveStepProfile(context.stepProfiles, resp.step_id) });
      } catch (err) {
        if (err instanceof AgentTimeoutError) {
...
export async function startFresh(stratum, specYaml, featureCode, description, dataDir, templateName, mode = 'feature', preMergeGate, roles, workspaceRoot) {
  ...
  writeActiveBuild(dataDir, {
    ...
    mode,
    pid: process.pid,
    currentStepId: response.ready?.[0]?.id ?? response.step_id,
    specPath: `pipelines/${templateName}.stratum.yaml`,
    stepNum: response.step_number ?? 1,
    totalSteps: response.total_steps ?? null,
    retries: 0,
...
export async function abortBuild(dataDir, featureCode, cwd, opts = {}) {
  ...
  // C2: resolve the engine from the PROJECT ROOT that dataDir represents (threaded
  // by the caller), NOT process.cwd(). The server route switches projects at
  // runtime without chdir, so a python-pinned project must audit through the
  // python engine — resolving from process.cwd() would audit through the wrong
  // engine and miss the abort.
  const stratum = opts.stratum ?? new StratumMcpClient();
  try {
    await stratum.connect(resolveStratumMcpConnection(cwd));
