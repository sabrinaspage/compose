 ...
 */
function resolveStepOutputContract(spec, flowName, stepId) {
  const steps = spec?.flows?.[flowName]?.steps;
  if (!Array.isArray(steps)) return { hasOutContract: false, outputFields: {} };
  const contractName = steps.find(step => step?.id === stepId)?.out;
  const contract = contractName ? spec?.contracts?.[contractName] : null;
  return {
    hasOutContract: typeof contractName === 'string' && contractName.length > 0,
...
async function runConsumerIssuance({
  descriptor,
  flowId,
  stratum,
  artifacts,
  ...
  try {
    ...
    const response = await stratum.stepDone(
      flowId, descriptor.id, envelope, descriptor.epoch, descriptor.dispatchToken,
    );
    ...
    artifacts.reconcileAudit(await stratum.audit(flowId));
    return response;
  }
  ...
  const prompt = buildStepPrompt(dispatch, worktreeContext);
  const maxDurationMs = STEP_TIMEOUT_MS[descriptor.id] ?? DEFAULT_TIMEOUT_MS;
...
export async function runBuild(featureCode, opts = {}) {
  ...
  try {
    ...
    while (!isTerminalFlow(response.status)) {
      ...
      if (isConsumerDescriptor(readyStep)) {
        const flowId = response.runId ?? response.flow_id;
        const runPins = { revisionDigest: readyStep.revisionDigest, specDigest: localSpecDigest };
        // The journal is created (if new) already pinned with the run revision +
        // spec fingerprint — the pins land in its first durable write, not a later
        // one, so a crash before bind cannot leave an unpinned journal (R1).
        const artifacts = artifactsForRun(flowId, runPins);
        if (typeof artifacts.hooks.beforeRevisionBind === 'function') {
          await artifacts.hooks.beforeRevisionBind({ descriptor: readyStep, journal: artifacts.journal });
        }
        // bindRunRevision now mainly asserts each descriptor carries the run's
        // revision (and pins on the legacy path where the constructor had none).
        artifacts.bindRunRevision(runPins);
        const audit = await stratum.audit(flowId);
        await visionWriter.updateItemPhase(itemId, readyStep.id);
        updateActiveBuildStep(dataDir, readyStep.id, {
          stepNum: response.step_number,
          totalSteps: response.total_steps,
        });
        response = await runConsumerIssuance({
          descriptor: readyStep,
          flowId,
          stratum,
          artifacts,
          audit,
      ...
      if (response.status === 'ready' || response.status === 'execute_step') {
        ...
        if (shouldInterceptShip(stepId, mode)) {
          ...
          response = await stratum.stepDone(
            flowId, stepId, shipResult, readyStep?.epoch, readyStep?.dispatchToken,
          );
          streamWriter.write({
            type: 'build_step_done',
        ...
        response = await stratum.stepDone(
          flowId, stepId, stepDoneResult, readyStep?.epoch, readyStep?.dispatchToken,
        );
...
export async function startFresh(stratum, specYaml, featureCode, description, dataDir, templateName, mode = 'feature', preMergeGate, roles) {
  const flowName = extractFlowName(specYaml, templateName);
...
export async function abortBuild(dataDir, featureCode) {
  const active = readActiveBuild(dataDir);
  if (!active) {
