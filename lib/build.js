    ...
    while (!isTerminalFlow(response.status)) {
      ...
      if (response.status === 'ready' || response.status === 'execute_step') {
        ...
        if (shouldInterceptShip(stepId, mode)) {
          ...
          response = await stratum.stepDone(flowId, stepId, shipResult, readyStep?.epoch);
          streamWriter.write({
            type: 'build_step_done',
        ...
        response = await stratum.stepDone(flowId, stepId, stepDoneResult, readyStep?.epoch);
        ...
        if (waitingGates.length > 1) {
          throw new Error(`build.js gate seam: expected a single waiting gate, found ${waitingGates.length}: ${waitingGates.join(', ')}`);
        }
        const gateStepId = waitingGates[0];
        ...
        if (policy.mode === 'skip') {
          ...
          // STRAT-PY-RETIRE: the TS engine has no `killed` status — an on_kill:null gate
          // abort returns terminalFailure (status 'failed'). Remap a deliberate human kill
          // so the existing `killed` terminal branch records an aborted build, not a failure.
          if (outcome === 'kill' && (response.status === 'failed' || response.status === 'budget_exhausted')) {
            response = { ...response, status: 'killed' };
          }
