...
export async function runBuild(featureCode, opts = {}) {
  ...
  const finalizeBuildAttempt = () => {
    ...
    if (accumulator && accumulator.build_id === build_id) {
  ...
  try {
    ...
    // threw above without persisting anything). Persist the candidate, rotate a
    // stale identity on fresh-over-failed, and emit this attempt's estimate.
    if (isFreshStart && !isNewAccumulator) {
      // A fresh start must not inherit a failed attempt's identity: that
      // build_id's terminal row already closed its story. Rotate, and reset the
      // in-memory seeds taken from the stale record (review r2).
