 ...
 *
 * Reads the TS store first (`STRATUM_STATE_ROOT/<flowId>.json` when configured,
 * otherwise `~/.stratum/ts/flows/<flowId>.json`) and falls back to the legacy
 * Python store (`~/.stratum/flows/<flowId>.json`).
 *
 ...
 *
 * Fresh TS runs have no `rounds` field and use round 0. If the TS state cannot
 * be read, fall back to the Python store's `round` field and legacy default 1.
 * A read failure must never block a gate.
 *
 * @param {string} flowId
 * @returns {number}
 */
export function readFlowRound(flowId) {
  ...
  } catch { /* fall through to the legacy Python store */ }
  // Legacy Python store (pre-cutover callers).
  try {
    const state = JSON.parse(readFileSync(join(homedir(), '.stratum', 'flows', `${flowId}.json`), 'utf-8'));
    const r = state?.round;
