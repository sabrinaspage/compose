...
// COMP-TRIAGE-5: front-of-pipeline scope-estimation vocab. `lane` drives which
// phases get skipped; `estimateSource` records whether the lane came from the
...
/**
 * Validate the subset of feature.json fields that carry a fixed vocabulary or
 * range. Called from both addRoadmapEntry (below) and the build write path
 * (lib/build.js) so neither can bypass the guard the way the old inline
 * `COMPLEXITIES.has` check — scoped only to addRoadmapEntry — allowed
 * `complexity: String(triageResult.tier)` (e.g. "0".."4") to reach disk via a
 * raw provider write.
 *
 * Every field is optional; only fields present on `fields` are checked.
 * Throws a plain Error (matching addRoadmapEntry's existing signaling) on the
 * first violation found.
 *
 * @param {object} fields
 * @param {string} [fields.complexity]     one of S|M|L|XL
...
export function validateFeatureFields(fields) {
  if (fields.complexity !== undefined && !COMPLEXITIES.has(fields.complexity)) {
    throw new Error(`feature-writer: invalid complexity "${fields.complexity}"`);
  }
  if (fields.triageTier !== undefined &&
      (!Number.isInteger(fields.triageTier) || fields.triageTier < 0 || fields.triageTier > 4)) {
    throw new Error(`feature-writer: invalid triageTier "${fields.triageTier}" — must be an integer 0-4`);
  }
  if (fields.lane !== undefined && !LANES.has(fields.lane)) {
    throw new Error(`feature-writer: invalid lane "${fields.lane}" — must be one of ${[...LANES].join(', ')}`);
  }
  if (fields.estimateSource !== undefined && !ESTIMATE_SOURCES.has(fields.estimateSource)) {
    throw new Error(
...
// AUDIT-19: a writer result must never carry the full regenerated ROADMAP.
// roundtripGuard now returns only small diagnostic fields, but a persistent
// idempotency-cache entry written before that change can still hold
// roundtrip.canonical (the entire markdown), so re-sanitize on the way out —
// covering both the fresh and cache-hit paths, and any future roundtrip writer.
// No-op for results that don't carry roundtrip.canonical.
function sanitizeWriterResult(result) {
  if (result && result.roundtrip && typeof result.roundtrip === 'object' && 'canonical' in result.roundtrip) {
    const { canonical, ...safe } = result.roundtrip;
    return { ...result, roundtrip: safe };
  }
  return result;
}
...
function maybeIdempotent(args, fn) {
  if (args.idempotency_key) {
    return checkOrInsert(args.cwd, args.idempotency_key, fn).then(({ result }) => sanitizeWriterResult(result));
  }
  return Promise.resolve().then(fn).then(sanitizeWriterResult);
}
...
export async function addRoadmapEntry(cwd, args) {
  ...
  // COMP-TRIAGE-5: complexity/triageTier/lane/estimateSource shape validation
  // lives in the shared validateFeatureFields guard so the build write path
  // (lib/build.js) can call the same check and can't bypass it the way the
  // old inline `COMPLEXITIES.has` (scoped only to this function) allowed.
  validateFeatureFields(args);
  ...
  return maybeIdempotent({ ...args, cwd }, async () => {
    ...
    // COMP-TRIAGE-5: front-of-pipeline scope-estimation fields.
