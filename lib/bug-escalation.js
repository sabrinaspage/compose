...
function escalationTelemetry(context) {
  const telemetry = {
    site: 'escalation',
  ...
  for (const [field, value] of [
    ...
    ['step_id', context.step_id ?? context.currentStepId],
  ]) {
    if (typeof value === 'string' && value.length > 0) telemetry[field] = value;
  }
  if (typeof context.attempt === 'number' && Number.isFinite(context.attempt)) {
    telemetry.attempt = context.attempt;
  }
  return telemetry;
}
...
export async function tier1CodexReview(stratum, context, bugDescription, reproTest, currentDiff, hypotheses) {
  ...
  const rawText = await stratum.runAgentText('codex', codexPrompt, {
    cwd: context.cwd,
    telemetry: escalationTelemetry(context),
  });
...
const WORKTREE_BASE = process.env.NODE_TEST_CONTEXT
  ? join(tmpdir(), 'compose-test-worktrees', 'comp-fix-hard')
  : join(homedir(), '.stratum', 'worktrees', 'comp-fix-hard');
...
export async function tier2FreshAgent(stratum, context, codexReview, hypotheses, checkpointPath) {
  ...
  try {
    ...
    const agent_reasoning = await stratum.runAgentText('claude', prompt, {
      cwd: wtPath,
      telemetry: escalationTelemetry(context),
    });
