...
5. **Value** them against `OBJECTIVE.md`. Never hand back an unranked list (`UNRANKED-IS-A-LIST`).
...
2. For each joint, write both branches: **if true → …** / **if false → …** (`JOINTS-ARE-BRANCHES`). If the two branches are identical, delete the joint.
3. Tag resolution method: `EXT` (look it up) · `CONSTRUCT` (build the test) · `ASSERT` (owner's call) · `STRADDLE` (build both branches).
4. Estimate cost in coarse buckets — hours / days / weeks / months (`COARSE-BUCKETS`).
...
5. Check accumulation: do several recorded weak signals now jointly cross the threshold (`SIGNALS-ACCUMULATE`)?
...
**Primary metric throughout:** `JOINT-RECALL` — when something later went wrong, was the cause on the list beforehand?
