...
- **R3 (F3 — real stamping topology).** There is NO single mutation/rollback wrapper. Record+manifest publication must be centralized across **every** record-durability boundary — the `commitWithProjections` path, the pending-intent persist path, intent publish/refuse/replay — enumerated as the plan's first task (Codex cited `judgment-writer.js:212, 836, 3144`). Projection-only regeneration (`regenerateProjections`, called by `getJudgmentState` at `judgment-writer.js:3303`) must NEVER stamp — else a read blesses a raw edit. **CORRECTED BY TASK 0 (topology §D):** "read-only" was wrong about `getJudgmentState` as a whole. It runs `replayIntentsLocked` at its head, so when pending intents exist a nominal read legitimately writes records and MUST stamp. The invariant is narrower than first stated: the projection/aggregation half must never stamp, the replay half must. A legit mutation path that fails to stamp = a false "forged" verdict, so this is a correctness requirement, not only a security one.
...
- Staging: v1 relies on verify-before-commit failing on drift, not re-plumbing `featureFiles` staging (a pre-staged judgment edit is caught by verify, not by staging changes). **AMENDED after the whole-branch review:** true only while the index and working tree agree. The verifier reads the working tree, `git commit` ships the index, so staging a forged record and then restoring the worktree copy verified GREEN and committed the forgery (reproduced by probe). The ship gate now refuses when any STAGED path under `docs/judgment/**` differs from the worktree bytes it just verified. A guarded path modified but not staged is untouched by this check — it is not being committed, and worktree drift is what the verifier already covers.
...
- [ ] `getJudgmentState`'s **projection/aggregation half** never mutates the manifest; its `replayIntentsLocked` head, which publishes pending intents, does stamp (R3, corrected by topology §D — the original "read-only, never mutates" wording was false and would drive a regression).
...
- [ ] **Cross-repo ship is out of scope.** The ship gate verifies the workspace's canon (`cwd`); when a build commits into a different repo (`agentCwd !== cwd`), that repo's own judgment canon is not verified. Left deliberately: verifying the target would hard-fail any target carrying `docs/judgment/**` without a baseline, and no cross-repo build was available to test that change against. The staged-divergence check does run against the committed tree.
